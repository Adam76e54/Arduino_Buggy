...
constexpr Coefficients MANUAL_COEFFICIENTS = {1,0,0};
constexpr Coefficients TRACK_COEFFICIENTS = {1,0,0};

float integralLeft = 0, integralRight;
float lastErrLeft = 0, lastErrRight = 0;
unsigned long lastTimeLeft = 0, lastTimeRight = 0;
...
void loop(){
  ...
  if(GUI && GUI.connected()){
    GUI.print("Left speed percentage = ");
    GUI.println(state::leftSpeedPercentage);
  }
}
...
float pidUpdate(float target, float measured, unsigned long &lastTime, float &lastErr, float &integral, const Coefficients& coeffs) {
  ...
  float output = coeffs.KP * error + coeffs.KI * integral + coeffs.KD * derivative;
...
void manualMode(){
  ...
  state::leftSpeedPercentage = pidUpdate(state::targetLeftRPM, leftRPM, lastTimeLeft, lastErrLeft, integralLeft, MANUAL_COEFFICIENTS);
  state::rightSpeedPercentage = pidUpdate(state::targetRightRPM, rightRPM, lastTimeRight, lastErrRight, integralRight, MANUAL_COEFFICIENTS);
